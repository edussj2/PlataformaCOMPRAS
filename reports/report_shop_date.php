<?php
    ...
    if(isset($_POST['fecha1_reporte_compra']) && isset($_POST['fecha2_reporte_compra'])){
		$fecha1 = $_POST['fecha1_reporte_compra'];
        $fecha2 = $_POST['fecha2_reporte_compra'];
	}else{
		$fecha1 = '2006-01-01';
        $fecha2 = '2025-01-01';
	}
    ...
    INNER JOIN caja ON compra.idCaja = caja.idCaja WHERE compra.fecha >= '$fecha1' AND compra.fecha <= '$fecha2' ORDER BY fecha DESC";
	...
	$objPHPExcel->getProperties()->setCreator("Sistema Pernos & Pernos")->setDescription("Reporte de compras por Fechas");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B3', 'REPORTE DE COMPRAS POR FECHA');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('G6', 'TRABAJADOR');
	...
	while($rows = $resultado->fetch()){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('G'.$fila, $rows['nombres']." ".$rows['apellidos']);
	...
	header('Content-Disposition: attachment;filename="shop-date.xlsx"');
